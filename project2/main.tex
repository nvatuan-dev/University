...
\begin{document}
    ...
    \vspace{10mm}
    \begin{flushleft}
        RADAR is an electromagnetic system for the detection and location of target objects such as
    ...
    \begin{figure}[ht]
        \centering
        \includegraphics[scale=0.7]{img/block_diagram.png}
        \caption{Block diagram for radar model}
    \end{figure}
    \linebreak
    ...
    \subsection{Structure Project}
        \begin{figure}[H]
            \centering
            \includegraphics[width=0.8\linewidth]{img/flowchart1.png}
            \caption{Flowchart for project}
        \end{figure}
        \large{
                    Arduino board sends a signal of 5V to TRIG pin of Ultrasonic Sensor HC-SR04 
                which triggers the sensor. Then it provides rotational action at the servo motor mechanically 
                fitted along with Ultrasonic sensor so that it can detect the moving objects and locate within 
                180 degrees. \\ 
                \vspace{1mm}
                \par The Arduino sends a HIGH pulse width on the TRIGGER pin of the sensor to 
                ...
                detects the width of the pulse to calculate the distance. \\
                \vspace{1mm}
                ...
                \vspace{1mm}
                \par The LCD displays the calculated distance and the angle of rotation. The buzzer 
                ...
                \vspace{2mm}
                \par Project have a formula to calculate the distance is:
                \begin{align*}
                    \centering
                    \textbf{S = V*t}
                \end{align*}
                \par \textbf{Which is:} 
                \begin{itemize}
                    \item \textbf{S:} Distance from the sensor to the object.
                    \item \textbf{V:} velocity, which is 340m/s.
                    \item \textbf{t:} Width of Echo Pulse, in $\mu$S (micro second).
                    \item Distance in centimeters = Time / 58.
                    \item Distance in Inches = Time / 148
                \end{itemize}
                \vspace{2mm}
                \par In this project, will have to convert this time into centimeters to calculate the distance traveled. 
                We will use the following equation to calculate it.
                The ultrasonic wave is basically a sound wave that travels at a speed of 340m/s 
    ...
    \subsection{Theory about the radar}
        ...
        in the ambience. For the first time, in the year 1940
        US Navy coined the term RADAR. This system operated by transmitting an electromagnetic wave and
        detecting the reflected signal. Earlier, the RADAR was mainly developed for detecting the existence of a
        target for measuring its range, so addressed as RADAR. During the Second World War, it was first used to
        notify the approach of hostile aircraft and for routing anti aircraft weapons. The modern RADAR system
        can be used to extract much more information from the reflected signal and got broader applications but
        still the range detection is one of its important functions. Till now, there is no electronic system which can
        replace the RADAR for its accuracy and efficiency in sensing and detection. \\
        \vspace{1mm}
        \par Radar works in much the same way. The word "Radar" is stands for \textbf{RA}dio \textbf{D}etection 
    ...
    \subsection{Theory concepts related to research issues}
        ...
        make up a computer program.
            \item \textbf{USB(Universal Serial Bus)}: is an industry standard that establishes specifications for
        cables and connectors and protocols for connection, communication and powersupply(interfacing) between
        computers, peripheral and other computers.
        \end{itemize}
    ...
    \subsection{Component Description}
        ...
        \subsubsection{\Large{Arduino Uno}}
            ...
            \begin{enumerate}
                ...
                the real world. \\
                \vspace{3mm}
                \par Apart from USB, battery or AC to DC adapter(wall-wart) or battery can also be used 
                to power the board. The adapter can be connected by plugging a 2.1mm center-positive plug 
                into the board's power jack. Leads from a battery can be inserted in the GND and Vin pin 
                headers of the POWER connector. \\
                \vspace{3mm}
                \par The board can operate on external supply from 6 to 20 volts. If supplied with less than 7V, 
                however, the 5V pin may supply less than 5V and the board may become unstable. If using 
                ...
                where RAM memory is 32KB(with 0.5 KB occupied by the bootloader). 
                \begin{figure}[H]
                    \centering
                    \includegraphics[width=0.5\linewidth]{img/Uno.png}
            ...
            \newpage
                \item \textbf{Features}
                    ...
                    \begin{figure}[H]
                        \centering
                        \includegraphics[width=0.6\linewidth]{img/atmega.png}
        ...
        \subsubsection{\Large{Servo Motor}}
            \begin{enumerate}
                ...
                such as DVD players use servos to extend or retract the disc trays.
                    \begin{figure}[H]
                        \centering
                        \includegraphics[width=0.6\linewidth]{img/Servo.jpg}
                        \caption{Assortment of Servos}
                    \end{figure}
                ...
                used to do work you apply torque when you open a bottle).
                    \begin{figure}[H]
                        \centering
                        \includegraphics[width=0.6\linewidth]{img/Servo1.jpg}
                        \caption{The Gears in a Typical Standard-size servo}
                    \end{figure}
                    \newpage
                    ...
                    \begin{figure}[H]
                        \centering
                        \includegraphics[width=0.6\linewidth]{img/Servo2.jpg}
                        \caption{The gears in a Typical High-power servo}
                    \end{figure} 
                    \vspace{2mm}
                ...
                position.
                    \begin{figure}[H]
                        \centering
                        \includegraphics[width=0.4\linewidth]{img/Servo3.jpg}
                        \caption{The circuit board and DC motor in a high-power servo}
                    \end{figure}
                ...
                \item \textbf{Selecting a servo motor}
                    ...
                    \begin{figure}[H]
                        \centering
                        \includegraphics[width=0.3\linewidth]{img/sg90.jpg}
                        \caption{Micro Servo 90}
                    \end{figure}
                    \newpage
                    ...
                    \begin{figure}[H]
                        \centering
                        \includegraphics[width=\linewidth]{img/Capture.PNG}
                        \caption{Section of SG90}
                    \end{figure}
                    \par \textbf{Block Diagram of SG90}
                    \begin{figure}[H]
                        \centering
                        \includegraphics[width=0.6\linewidth]{img/Servo2.PNG}
                        \caption{SG90 datasheet}
                    \end{figure}
                \item \textbf{Controlling a servo motor} 
                    ...
                    \begin{figure}[H]
                        \centering
                        \includegraphics[width=0.4\linewidth]{img/servo3.PNG}
                        \caption{PWN Period}
                    \end{figure}
        ...
        \subsubsection{\Large{HC-SR04 (Ultrasonic Sensor)}}
            \begin{enumerate}
                ...
                \item HC-SR04 Pinout and Description
                    ...
                    \begin{table}[ht]
                        ...
                        \begin{tabular}{ | l | l | p{7cm} |}
                            ...
                            \hline
                        \end{tabular}
                        \caption{Pinout of HC-SR04}
                    \end{table}
                    \par This figure below here is labelled these HC-SR04 Pinout for better visualization:
                    \begin{figure}[H]
                        \centering
                        \includegraphics[width=\linewidth]{img/HC-SR04.jpg}
                        \caption{HC-SR04 Pinout}
                    \end{figure}
                \item \textbf{How does it work?}
                    \begin{figure}[H]
                        \centering
                        \includegraphics[width=0.4\linewidth]{img/ultrasonic_sensor.png}
                ...
                hit and bounce back from the object and then collected by the receiver of the module. \\
                    \begin{figure}[H]
                        \centering
                        \includegraphics[width=0.6\linewidth]{img/ultrasonic.png}
                        \caption{Principle of wave transmission}
                    \end{figure}
        ...
        \subsubsection{\Large{LCD 16x02}}
            \begin{enumerate}
                \item \textbf{Introduction}
                    \begin{figure}[H]
                        \centering
                        \includegraphics[width=0.4\linewidth]{img/LCD.jpg}
                        \caption{LCD 16x2 Module}
                    \end{figure}
                ...
                \item \textbf{Block Diagram}
                    \begin{figure}[H]
                        \centering
                        \includegraphics[width=\linewidth]{img/lcd diagram.PNG}
                        \caption{Block Diagram of LCD}
                    \end{figure}
        ...
        \subsubsection{\Large{Buzzer}}
            \begin{enumerate}
                \item \textbf{General Introduction}
                    \begin{figure}[H]
                        \centering
                        \includegraphics[width=0.4\linewidth]{img/buzzer.PNG}
                        \caption{Buzzer}
                    \end{figure}
                ...
                \item \textbf{Types of Buzzer}
                    \begin{itemize}
                        ...
                        \begin{figure}[H]
                            \centering
                            \includegraphics[width=0.3\linewidth]{img/buzzer1.jpg}
                            \caption{Electromechanical Buzzer}
                        \end{figure}
                        ...
                        \begin{figure}[H]
                            \centering
                            \includegraphics[width=0.4\linewidth]{img/buzzer2.jpg}
                            \caption{Mechanical Buzzer}
                        \end{figure}
                        ...
                        \begin{figure}[H]
                            \centering
                            \includegraphics[width=0.3\linewidth]{img/buzzer.jpg}
                            \caption{Piezoelectric Buzzer}
                        \end{figure}
                    \end{itemize}
                \item \textbf{Pin Configuration}
                    ...
                    \begin{figure}[H]
                        \centering
                        \includegraphics[width=0.4\linewidth]{img/buzzer-pinout.png}
    ...
    \input{chapter2.tex}
        \section{CONTENT}
                \subsection{\large{Block Diagram}}
                    \begin{figure}[H]
                        \centering
                        \includegraphics[width=0.8\linewidth]{img/diagram-project.png}
                        \caption{Arduino Radar Using Ultrasonic Sensor}
                    \end{figure}
                ...
                \subsection{\large{Working of the project}}
                    \begin{figure}[H]
                        \centering
                        \includegraphics[width=0.6\linewidth]{img/500px-PSR_1.png}
                        \caption{PSR principle of operation}
                    \end{figure}
                    ...
                    \par For the conventional radar, here is a summary of how radar works:
                        ...
                        \begin{figure}[H]
                            \centering
                            \includegraphics[width=0.8\linewidth]{img/radar-work.png}
                            \caption{How a radar works}
                        \end{figure}
        \section{CONNECT ARDUINO AND BOARD}
            \subsection{CIRCUIT DIAGRAM}
                \begin{figure}[H]
                    \centering
                    \includegraphics[width=0.8\linewidth]{img/circuit.jpg}
                    \caption{Circuit Diagram}
                \end{figure}
            \subsection{PROGRAM MY BOARD}
                In this project, unlike almost the tutorials on the Internet, i'll use Visual Studio Code 
                to embed code to my Arduino board which was written with C++ programming languague.  
                \begin{figure}[H]
                    \centering
                    \includegraphics[width=\linewidth]{img/program.png}
                    \caption{Embed code to the program}
                \end{figure}
        \section{ENFORCEMMENT RESULT}
            When the radar detect an objects within the safe range, a green notification 
            light will appear. 
            \begin{figure}[H]
                \centering
                \includegraphics[width=0.6\linewidth]{img/green.jpg}
                \caption{An object in the safe range}
            \end{figure}
            Other ways, if the radar detect an object within the dangerous range, a red notification 
            light will appear.
            \begin{figure}[H]
                \centering
                \includegraphics[width=0.6\linewidth]{img/red.jpg}
                \caption{An object in the dangerous range}
            \end{figure}
        ...
        \section{CONCLUDES}
            \subsection{\large{Advantages}}
                \begin{itemize}
                    ...
                    able to do.
                    \item The operating range of the sensor can scan in a range of distance of 2-400 cm.
                \end{itemize}
            \subsection{\large{Defects}}
                \begin{itemize}
                    \item Too much wire to connect all the component.
                    \item The components of this project are quite expensive.
                \end{itemize}
